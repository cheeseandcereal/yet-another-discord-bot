...
# Check and try to use python3.11 explicitly, if possible
...
if [ $# -lt 1 ]; then
    ...
    $py_exec -m coverage run --branch --source=./lib -m unittest discover -p '*utest.py'
elif [ "$1" = "coverage" ]; then
    include=$(find . -path "*.py" -not -path "*.git*" -not -path "*.mypy_cache*" -not -path "*.venv*" | tr '\n' ',' | rev | cut -c 2- | rev)
    $py_exec -m coverage report -m --include="$include"
elif [ "$1" = "tests" ]; then
    sh tools.sh unit
    sh tools.sh coverage
elif [ "$1" = "lint" ]; then
    find . -path "*.py" -not -path "*.git*" -not -path "*.mypy_cache*" -not -path "*.venv/*" -exec $py_exec -m flake8 {} +
    $py_exec -m black --check -l 150 -t py311 bot.py lib/
elif [ "$1" = "format" ]; then
    $py_exec -m black -l 150 -t py311 bot.py lib/
elif [ "$1" = "clean" ]; then
